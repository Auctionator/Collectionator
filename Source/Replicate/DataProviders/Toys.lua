...
CollectionatorReplicateToyDataProviderMixin = CreateFromMixins(AuctionatorDataProviderMixin)

function CollectionatorReplicateToyDataProviderMixin:OnLoad()
  ...
  Auctionator.EventBus:RegisterSource(self, "CollectionatorReplicateToyDataProvider")

  self.dirty = false
  self.toys = {}
end

function CollectionatorReplicateToyDataProviderMixin:OnShow()
...
function CollectionatorReplicateToyDataProviderMixin:ReceiveEvent(eventName, eventData, eventData2)
...
function CollectionatorReplicateToyDataProviderMixin:Sort(fieldName, sortDirection)
...
function CollectionatorReplicateToyDataProviderMixin:Refresh()
  if self.dirty then
  ...
  local filtered = Collectionator.Utilities.ReplicateExtractWantedItems(Collectionator.Utilities.ReplicateGroupedByID(self.toys, self.fullScan), self.fullScan)
  local results = {}
...
function CollectionatorReplicateToyDataProviderMixin:UniqueKey(entry)
  return tostring(entry.index)
end

function CollectionatorReplicateToyDataProviderMixin:GetTableLayout()
  return TOY_TABLE_LAYOUT
end

Auctionator.Config.Create("COLLECTIONATOR_COLUMNS_TOY", "collectionator_columns_toy", {})

function CollectionatorReplicateToyDataProviderMixin:GetColumnHideStates()
  return Auctionator.Config.Get(Auctionator.Config.Options.COLLECTIONATOR_COLUMNS_TOY)
end


function CollectionatorReplicateToyDataProviderMixin:GetRowTemplate()
