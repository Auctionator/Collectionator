...
CollectionatorReplicatePetDataProviderMixin = CreateFromMixins(AuctionatorDataProviderMixin)

function CollectionatorReplicatePetDataProviderMixin:OnLoad()
  ...
  Auctionator.EventBus:RegisterSource(self, "CollectionatorReplicatePetDataProvider")

  self.dirty = false
  self.pets = {}
end

function CollectionatorReplicatePetDataProviderMixin:OnShow()
...
function CollectionatorReplicatePetDataProviderMixin:ReceiveEvent(eventName, eventData, eventData2)
...
function CollectionatorReplicatePetDataProviderMixin:Sort(fieldName, sortDirection)
...
local function ReplicateGroupedByIDLevelAndQuality(array, fullScan)
  local results = {}
...
function CollectionatorReplicatePetDataProviderMixin:Refresh()
  if self.dirty then
  ...
  local filtered = Collectionator.Utilities.ReplicateExtractWantedItems(ReplicateGroupedByIDLevelAndQuality(self.pets, self.fullScan), self.fullScan)
  local results = {}
...
function CollectionatorReplicatePetDataProviderMixin:UniqueKey(entry)
  return tostring(entry.index)
end

function CollectionatorReplicatePetDataProviderMixin:GetTableLayout()
  return PET_TABLE_LAYOUT
end

Auctionator.Config.Create("COLLECTIONATOR_COLUMNS_PET", "collectionator_columns_pet", {})

function CollectionatorReplicatePetDataProviderMixin:GetColumnHideStates()
  return Auctionator.Config.Get(Auctionator.Config.Options.COLLECTIONATOR_COLUMNS_PET)
end

function CollectionatorReplicatePetDataProviderMixin:GetRowTemplate()
  return "CollectionatorReplicatePetRowTemplate"
end
